...
const totalBeds = computed(() => {
  return emptyBeds.length + students.value.filter(s => s.status === "入住中").length
})

const occupiedCount = computed(() => {
  return students.value.filter(s => s.status === "入住中").length
})

const occupancyRate = computed(() => {
  if (totalBeds.value === 0) return 0
  return Math.round((occupiedCount.value / totalBeds.value) * 100)
})

const emptyBedRate = computed(() => {
  if (totalBeds.value === 0) return 0
  return Math.round((emptyBeds.length / totalBeds.value) * 100)
})

const repairDoneRate = computed(() => {
  if (repairs.value.length === 0) return 0
  const done = repairs.value.filter(r => r.status === "已完成").length
  return Math.round((done / repairs.value.length) * 100)
})
...
function updateRepairStatus(index) {
  const newStatus = prompt(
    "請輸入狀態：待處理 / 處理中 / 已完成",
    repairs.value[index].status
  )
  ...
  if (
    ...
    repairs.value[index].status = newStatus
...
function deleteRepair(index) {
  const result = confirm("確定要刪除這筆報修案件嗎？")
...
function createCheckin() {
  ...
  student.status = "入住中"
  student.bed = checkinForm.value.bedId
...
function resetData() {
  const result = confirm("確定要重置所有資料嗎？")
...
/>
        ...
        <div class="user">
          <span>幫助中心</span>
          <button @click="exportData">匯出數據</button>
...
<td>
  <button
    ...
    @click="updateRepairStatus(index)"
  >
    更新狀態
  </button>
